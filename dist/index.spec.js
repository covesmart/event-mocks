...
describe("creating a new SNS event", function () {
    it("should return a valid event", function () {
        ...
        chai_1.expect(lodash_1.get(event, "Records[0].Sns.Message")).to.equal("trigger-email");
        chai_1.expect(lodash_1.get(event, "Records[0].Sns.Type")).to.equal("Notification");
    });
});
describe("createSqsEvent()", function () {
    it("should return SQS mocked event", function () {
        ...
        chai_1.expect(lodash_1.get(event, "Records[0].body")).to.equal('{"foo":"bar"}');
        chai_1.expect(lodash_1.get(event, "Records[0].eventSource")).to.equal("aws:sqs");
    });
});
...
describe("createWebsocketEvent()", function () {
    it("should return websocket mocked event", function () {
        ...
        chai_1.expect(parsedBody.first_name).to.equal("Sam");
        chai_1.expect(parsedBody.last_name).to.equal("Smith");
        chai_1.expect(lodash_1.get(event, "requestContext.connectedAt")).to.equal(123);
        chai_1.expect(lodash_1.get(event, "requestContext.connectionId")).to.equal("abc123");
    });
});
describe("createS3Event()", function () {
    it("should return S3 mocked event", function () {
        ...
        chai_1.expect(lodash_1.get(event, "Records[0].s3.bucket.name")).to.equal("my-bucket-name");
        chai_1.expect(lodash_1.get(event, "Records[0].s3.object.key")).to.equal("object-key");
        chai_1.expect(lodash_1.get(event, "Records[0].eventName")).to.equal("ObjectCreated:Put");
    });
    it("should return S3 mocked event without side-effect", function () {
        ...
        chai_1.expect(lodash_1.get(event, "Records[0].s3.bucket.name")).to.equal("my-bucket-name");
        chai_1.expect(lodash_1.get(event, "Records[0].s3.object.key")).to.equal("object-key");
        chai_1.expect(lodash_1.get(event2, "Records[0].s3.object.key")).to.equal("object-key-2");
        chai_1.expect(lodash_1.get(event, "Records[0].eventName")).to.equal("ObjectCreated:Put");
    });
});
...
describe("createKinesisEvent()", function () {
    it("should return Kinesis mocked event", function () {
        ...
        chai_1.expect(Buffer.from(lodash_1.get(event, "Records[0].kinesis.data"), "base64").toString("ascii")).to.equal("kinesis test");
    });
});
...
describe("createAlexaSkillEvent()", function () {
    it("should return a valid event", function () {
        ...
        chai_1.expect(lodash_1.get(event, "request.type")).to.equal("CanFulfillIntentRequest");
        chai_1.expect(lodash_1.get(event, "context.System.device.deviceId")).to.equal("myDevice");
    });
});
describe("createAlexaSmartHomeEvent()", function () {
    it("should return a valid event", function () {
        ...
        chai_1.expect(lodash_1.get(event, "payload.switchControlAction")).to.equal("TURN_OFF");
    });
});
